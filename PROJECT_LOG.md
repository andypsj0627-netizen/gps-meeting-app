...
### 완료 (실시간 위치→조우 공고화 — 등급제 엔진 착수 전 기반 다지기)
- **조우 정의를 "스침"에서 "체류"로 격상** (사용자 결정): 반경을 스치는 한 틱에 즉시 확정하던 것을, 진입 반경 안으로 들어와 해제 반경 안에서 **연속 dwell 이상 머물러야 확정**하도록 `EncounterDetector`를 2상태→3상태(pending 추가)로 재설계. GPS 노이즈·단순 통과를 걸러 이벤트 트래픽을 줄이고, 등급제 문서의 희소한 '운명' 트리거 포지셔닝과 정합. `encounterDwell` 상수(테스트 10초 / 출시 목표 5분, 반경 상수와 동일 "출시 전 되돌릴 것" 패턴). 기준선 억제 제거 — 앱 진입 시 이미 옆에 있던 쌍도 dwell 채우면 카운트. dwell 승급은 스트림 이벤트 구동(실제 presence 도입 시 주기 티커로 업그레이드 — ponytail 주석). detector 생성을 `encounterDetectorProvider` seam으로 분리해 테스트가 dwell을 override
- **시뮬레이션이 사람처럼 걷다-멈추다 반복**: 가상 사용자가 보행 중 낮은 확률(2%/틱)로 3~8초 멈췄다가 **같은 경로를 이어서 재개**(waiting 종료 시 남은 경로면 재개, 아니면 새 목적지). `pauseProbability` 주입 가능 — 테스트는 0으로 연속보행 불변식을 결정적으로 검증. 멈춤이 생겨야 dwell이 쌓여 조우 관측 가능
- **지도에 조우 영향 반경 원 시각화**: 내 위치·근처 사용자 각각에 진입 반경 원(`CircleLayer`, 미터 반경), 해금(확정 조우) 시 진하게 — 지금 누가 누구의 영향권에 들었는지 가시화
- 검증: `flutter test` **100건 통과**(결정적, 2회 확인), `dart analyze` 클린
- **위임 사고 교훈**: 병렬 Worker 3명이 같은 git 워킹트리를 공유하던 중, dwell 담당 Worker가 타 Worker(pause·circle) 변경을 "범위 밖 오염"으로 오판해 `git checkout`으로 되돌림 → 미커밋이라 git 복구 불가, Advisor가 diff·설계로 재구성. 파일 수정 병렬 위임은 worktree 격리/순차 필요(메모리 기록). 재구성 중 시드 취약 테스트를 `pauseProbability` 주입으로 견고화

### 다음 세션에서 할 일
- **조우 등급제 엔진** (docs/encounter-tiers.md Phase A): dwell로 확정된 조우를 순수 Dart 로컬 원장에 누적 → 등급 파생 → 스침 화면 결합(별도 작업)
- (별도) 실제 presence 백엔드(Firebase), GPS 정확도(accuracy) 게이팅, 시뮬레이션 속도 배율(5.0) 하향

---

## 2026-07-14

### 완료 (3차: 탭 화면 채우기 — Fake 데이터)
- 화면/기능 분리 원칙 첫 적용. Worker(Opus) 3명 병렬 위임, Advisor가 diff·테스트 직접 검증:
  - **스침**: 등급 enum(스침/인연/운명/연결, 엔진이 재사용할 라벨만) + 카드 뷰모델(가공 완료된 표시 문자열만 — 마스킹/포맷팅 로직 없음) + Fake 피드 6건 + 등급별 해금 렌더(스침=이니셜·나이대 흐리게, 인연=실명·한줄·맥락 강조, 운명='지금 근처' 뱃지, 연결=대화 열림) + 호감 토글
  - **대화**: 채팅 요약 모델 + Fake 2건(스침의 '연결' 등급 인물과 일치), 안읽음 뱃지, 탭 시 준비중 스낵바, 빈 상태
  - **마이**: Fake 내 프로필 provider(기존 NearbyUser 재사용, Firestore 교체 예정 명시) + 프로필 헤더 카드. 로그아웃 키/분기 무변경
- Fake 인물을 지도 시뮬레이션 가상 사용자(user1~10)와 통일 — 탭 간 세계관 일관
- 코드 리뷰(8앵글 → 후보 17건 → 검증 13건 → 확정 4건) 반영: 이니셜 `.characters.first` 빈 문자열 가드 2곳(실데이터 전환 대비), `_TierBadge`/`_NearbyBadge` → `_Badge` 병합, `_EmptyState` const화
  - 주요 기각: 빈 상태 UI/아바타/테스트 헬퍼 공용 추출(구조가 실제로 달라 간접층만 추가), 운명 뱃지 tier 게이트(문서상 운명 전용 연출 — 의도), 성별 라벨 중복('남' vs '남성' — 문맥별 의도가 다름)
- 미사용이 된 ComingSoonScreen 삭제, 화면 위젯 테스트 10건 신규, `flutter test` 98건 전체 통과

### 완료 (2차: 핵심 컨셉 확정 + 하단 탭 셸)
- **핵심 컨셉 확정** (README 재정의 + `docs/encounter-tiers.md` 신설):
  - 서비스 축을 둘로 정리: 실시간 조우(기존) + **시공간 겹침 등급제**(신규) — "같은 시간, 같은 공간을 얼마나 공유했나"를 단일 원장으로 누적해 관계 등급을 파생
  - 겹침 정의: 150m 셀 + 1시간 버킷, 역색인 데이터 모델, 프라이버시 가드레일, 구현 경로 Phase A/B/C
  - 대화는 상호 호감 게이트 뒤 원칙 명시, Phase 3 로드맵에 등급제/겹침 항목 추가
- **화면 구조 확정** (README): 하단 탭 5개 — 홈/지도/스침/대화/마이. 기본 진입은 지도, 홈은 플랫폼화 자리(당장은 빈 상태)
- **화면/기능 분리 개발 원칙** CLAUDE.md에 추가: 화면은 Fake 데이터(provider 주입)로, 엔진/로직은 순수 Dart + 단위 테스트로, 결합(Fake→실제 교체)은 별도의 작은 작업으로 (배경: 로그인 화면을 인증 배선과 한 번에 만들었다 무거워진 경험)
- **하단 탭 5개 셸 구현**: `StatefulShellRoute.indexedStack`으로 탭 상태 보존(지도 유지), 우회/인증 두 라우터 분기 모두 셸 적용(redirect 무변경), 스침/대화는 공용 `ComingSoonScreen`, 홈은 빈 상태, 로그아웃은 지도 AppBar→마이 탭으로 이사. 셸 테스트 5건 신규
- `flutter test` 88건 전체 통과

### 완료 (1차: 지도 개선 + 위치 데이터 모델)
...
### 다음 세션에서 할 일
- 조우 등급제 엔진 (docs/encounter-tiers.md Phase A): 순수 Dart 로컬 원장 + 등급 파생 — 스침 화면과의 결합은 그 다음 별도 작업
