...
void main() {
  ...
  List<List<NearbyUser>> collect(
    ...
    final sub = service.watchNearbyUsers(center).listen(emissions.add);
  ...
  group('FakeNearbyUsersService', () {
    ...
    test('폴리라인 세그먼트 경계를 넘어가면 다음 세그먼트로 이월하여 보간한다', () {
      fakeAsync((async) {
        ...
        // 사용자 A로 검증: 초기 위치 기준 굽은 경로를 따라 걸었는지 확인.
    ...
    test('구독 취소 후에는 더 이상 방출하지 않는다(타이머 정리)', () {
      fakeAsync((async) {
        ...
        final sub = service.watchNearbyUsers(center).listen(received.add);
