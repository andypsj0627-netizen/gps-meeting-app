...
/// 조우로 "해금"된 사용자 id를 누적하는 provider.
///
/// [encounterEventsProvider]가 방출하는 조우 배치를 구독하여, 각 이벤트의 두
/// 참가자 id(나 [EncounterDetector.selfId] 제외)를 집합에 쌓는다. 한 번 해금된
/// id는 계속 유지된다(sticky) — 조우가 해제되어도 프로필 열람 권한은 남는다.
...
class UnlockedUsersNotifier extends Notifier<Set<String>> {
  @override
  Set<String> build() {
    ref.listen(encounterEventsProvider, (_, next) {
      final events = next.value;
      if (events == null || events.isEmpty) return;
      final updated = {...state};
      for (final event in events) {
        if (event.a.id != EncounterDetector.selfId) updated.add(event.a.id);
        if (event.b.id != EncounterDetector.selfId) updated.add(event.b.id);
      }
      // 새로 해금된 id가 있을 때만 상태를 교체한다(불필요한 리빌드 방지).
