...
class AppConstants {
  ...
  /// 조우 확정에 필요한 최소 연속 체류 시간.
  ///
  /// 두 참가자가 진입 반경 안으로 들어온 뒤, 해제 반경 안에서 이 시간 이상
  /// 연속으로 함께 머물러야 조우로 확정한다. 잠깐 스쳐 지나가는(멈추지 않는)
  /// 보행이나 GPS 노이즈로 인한 순간적 겹침을 걸러낸다.
  ///
  /// 테스트 단계에서는 10초 체류만으로 조우를 관찰할 수 있게 짧게 둔다.
  /// 출시 전 Duration(minutes: 5)로 되돌릴 것.
  static const Duration encounterDwell = Duration(seconds: 10);
